...
<nav class="sticky top-10 z-50 bg-primary shadow">
  ...
  <!-- Mobile Menu -->
  <ul
    class="md:hidden fixed left-0 right-0 bottom-0 bg-white flex flex-col items-center pt-12 gap-5 shadow-lg transform transition-transform duration-300 z-40 top-[64px] -translate-x-full"
  ...
  >
    ...
    <li>
      <a href="/submission" class="text-gray-900 text-lg" on:click={closeMenu}>{$t('nav.submit')}</a
      >
    </li>
    <li>
    ...
    <li>
      <a href="/timeline" class="text-gray-900 text-lg" on:click={closeMenu}>{$t('nav.timeline')}</a
      >
    </li>
    <li>
